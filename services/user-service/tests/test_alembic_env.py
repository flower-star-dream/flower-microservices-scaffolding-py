...
@Author: 花海
@Date: 2026/08/16
@Description: 验证 user-service alembic/env.py 接入框架 load_env_file 后的数据库 URL 读取链路：
              .env 中的 USER_DATABASE_URL 能被自动加载（迁移命令单独执行时不依赖 shell
...
def _resolve_url() -> str | None:
    """模拟 user-service alembic/env.py _resolve_url() 的数据库 URL 读取链路：
    USER_DATABASE_URL > DATABASE_URL"""
    return os.environ.get("USER_DATABASE_URL") or os.environ.get("DATABASE_URL")


def test_load_database_url_from_env_file(tmp_path, monkeypatch):
    """.env 中的 USER_DATABASE_URL 可被自动加载（env.py 迁移链路依赖行为）"""
    monkeypatch.delenv("USER_DATABASE_URL", raising=False)
    monkeypatch.delenv("DATABASE_URL", raising=False)
    (tmp_path / ".env").write_text(
        "USER_DATABASE_URL=sqlite+aiosqlite:///./migration.db\n", encoding="utf-8"
    )
...
def test_service_specific_var_wins_over_generic(tmp_path, monkeypatch):
    """服务专属 USER_DATABASE_URL 优先于通用 DATABASE_URL"""
    monkeypatch.delenv("USER_DATABASE_URL", raising=False)
    monkeypatch.delenv("DATABASE_URL", raising=False)
    (tmp_path / ".env").write_text(
        "USER_DATABASE_URL=sqlite+aiosqlite:///./user.db\n"
    ...
    assert load_env_file() is True
    assert _resolve_url() == "sqlite+aiosqlite:///./user.db"


def test_existing_env_var_wins_over_env_file(tmp_path, monkeypatch):
    """进程/容器环境变量优先：已存在 USER_DATABASE_URL 时 .env 不覆盖（与框架 override=False 一致）"""
    monkeypatch.setenv("USER_DATABASE_URL", "mysql+aiomysql://root:prod@127.0.0.1:3306/prod_user")
    (tmp_path / ".env").write_text(
        "USER_DATABASE_URL=sqlite+aiosqlite:///./migration.db\n", encoding="utf-8"
    )
    monkeypatch.chdir(tmp_path)
    ...
    assert load_env_file() is True
    assert _resolve_url() == "mysql+aiomysql://root:prod@127.0.0.1:3306/prod_user"
...
def test_missing_env_file_returns_false(tmp_path, monkeypatch):
    """无 .env 文件时安全兜底：返回 False 且不抛异常（迁移仍可走 USER_DATABASE_URL 环境变量）"""
    monkeypatch.delenv("USER_DATABASE_URL", raising=False)
    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.chdir(tmp_path)
