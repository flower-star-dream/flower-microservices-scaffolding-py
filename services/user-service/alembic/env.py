"""
Alembic 迁移环境配置（user-service）

@Author: 花海
@Date: 2026/08/16
@Description: user-service 的 Alembic 迁移环境：支持异步 SQLAlchemy 引擎（mysql+aiomysql /
              sqlite+aiosqlite）与同步 URL（mysql+pymysql / sqlite）。
              数据库 URL 解析顺序：进程/容器环境变量 USER_DATABASE_URL > 通用 DATABASE_URL
              > 仓库根 .env（自动加载）> alembic.ini 的 sqlalchemy.url（ini 中留空），
              复用框架配置加载能力，避免硬编码数据库连接参数。
              业务模型已通过 user_service.model 导入注册到 Base.metadata（autogenerate 对比依据）。
              命令（仓库根执行）：alembic -c services/user-service/alembic.ini upgrade head
"""
...
# 服务 src 目录加入 sys.path（未以 editable 方式安装时也能导入 user_service / web_infra 包）
...
import user_service.model  # noqa: E402, F401
...
def _resolve_url() -> str:
    """解析迁移数据库 URL：进程/容器环境变量 USER_DATABASE_URL > 通用 DATABASE_URL
    ...
    load_env_file()
    url = (
        os.environ.get("USER_DATABASE_URL")
    ...
    if not url:
        raise RuntimeError(
            "缺少数据库 URL：请设置环境变量 USER_DATABASE_URL（或通用 DATABASE_URL）"
            "（如 mysql+aiomysql://root:xxx@127.0.0.1:3306/flower_user），"
