...
def _build_service(db, users: dict[int, dict] | None = None) -> tuple[OrderService, InMemoryMessageQueue]:
    """构造订单服务（注入 SQLite 内存库 + 假用户客户端 + 内存消息队列 + 内存缓存）"""
    mq = InMemoryMessageQueue()
    service = OrderService(
        repository=OrderRepository(db),
        user_client=_FakeUserClient(users),
        publisher=OrderEventPublisher(mq),
...
async def test_create_order_success(db):
    """创建订单成功：远程校验用户通过、订单落库、事件发布"""
    service, mq = _build_service(db)
    ...
    # 事件已发布（内存队列 pending 1 条）
    assert mq._queue.qsize() == 1


@pytest.mark.asyncio
async def test_create_order_user_not_found(db):
    """创建订单用户不存在（user-service 返回 404）：抛 COMMON_NOT_FOUND，且不落库"""
    service, mq = _build_service(db, users={})
    ...
    assert mq._queue.qsize() == 0  # 未发布事件
...
async def test_update_status_success(db):
    ...
    await service.update_status(vo.id, OrderConstant.ORDER_STATUS_CONFIRMED)
    updated = await OrderRepository(db).find_by_id(vo.id)
    assert updated is not None
    assert updated.status == OrderConstant.ORDER_STATUS_CONFIRMED
...
async def test_order_event_consumer_idempotent(db):
    ...
    publisher = OrderEventPublisher(mq)
    order = OrderModel(order_no="N10001", user_id=1, amount=Decimal("50.00"), status=1)
    await publisher.publish_created(order)
    ...
    await mq.publish(
        Message(
            ...
            body={"biz_id": str(order.id), "order_id": str(order.id), "order_no": "N10001"},
