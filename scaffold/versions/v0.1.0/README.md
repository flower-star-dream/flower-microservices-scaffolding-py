...
# 2) 安装框架依赖（默认 Git 远程拉取；本机已 clone 框架时可改用 pyproject.toml 注释中的方式二）
...
- **派生新项目**：GitHub "Use this template"（或手动 clone 清 git 历史）→ `python scripts/new_project.py new my-project`。
